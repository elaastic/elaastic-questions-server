...
const choices = computed(() => {
  if (props.response.questionType === 'MultipleChoice') {
    if(props.response.choices.length > 1){
      return `${t('responses')} [${(props.response as MultipleChoiceResponse).choices}]`
    }
    else{
      return `${t('response')} [${(props.response as MultipleChoiceResponse).choices}]`
    }
...
<template>
  ...
  <v-card :class="['card', providedByTeacher ? 'teacher-bg' : 'default-bg']">
    <v-card-title class="d-flex align-center">
      ...
      <div class="ms-auto">
        <v-tooltip v-if="bestAnswerLocal" :text="t('remove-from-best-answers')" location="bottom">
        ...
        <v-tooltip v-if="!bestAnswerLocal" :text="t('add-to-best-answers')" location="bottom">
          <template v-slot:activator="{ props }">
