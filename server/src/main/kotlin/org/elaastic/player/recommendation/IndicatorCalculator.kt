...
object IndicatorCalculator {
    ...
    fun computePConf(responseList: List<Response>?): Float? {
        ...
        val correctAnswers =
            responseList?.filter { response -> response.score?.toInt() == 100 && response.confidenceDegree != null }
        val incorrectAnswers =
            responseList?.filter { response -> response.score?.toInt() != 100 && response.confidenceDegree != null }
        val correctAnswersMeanConfidenceDegree =
            correctAnswers?.map { response -> response.confidenceDegree!!.ordinal }?.average()
        val incorrectAnswersMeanConfidenceDegree =
            incorrectAnswers?.map { response -> response.confidenceDegree!!.ordinal }?.average()
