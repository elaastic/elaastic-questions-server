...
### `idp-saml2`
This directory contain an Identity Provider that uses SAML2 protocol.
Its implemented by another Keycloak server.
...
### CAS server
Elaastic support the CAS protocol ; many CAS servers can be configured on Elaastic.
...
and then opens a session on `elaastic-iam` using SAM2, which turns to open a session on `elaastic` using OpenID Connect.